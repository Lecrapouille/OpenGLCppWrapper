...
IndexedVBO::IndexedVBO()
  : m_sphere("VAO_sphere"),
    m_prog("prog")
{
  std::cout << "Hello IndexedVBO" << std::endl;
}

IndexedVBO::~IndexedVBO()
{
  std::cout << "Bye IndexedVBO" << std::endl;
}

//------------------------------------------------------------------------------
//! \brief Callback when the window changed its size.
//------------------------------------------------------------------------------
void IndexedVBO::onWindowSizeChanged()
{
...
//------------------------------------------------------------------------------
bool IndexedVBO::setup()
{
  // Enable some OpenGL states
  glCheck(glEnable(GL_DEPTH_TEST));

  // Load vertex and fragment shaders with GLSL code.
  m_vertex_shader.fromFile("01_Core/shaders/07_IndexedVBO.vs");
  m_fragment_shader.fromFile("01_Core/shaders/07_IndexedVBO.fs");
